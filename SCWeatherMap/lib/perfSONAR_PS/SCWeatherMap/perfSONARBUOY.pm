package perfSONAR_PS::SCWeatherMap::perfSONARBUOY;
...
sub run {
    ...
    foreach my $link (@{ $args->{current_links} }) {
        ...
        foreach my $measurement_parameters (@{ $link->{measurement_parameters} }) {
            ...
            $final_dstsrc_stats{jitter} = $srcdst_stats->{jitter} if (defined $srcdst_stats->{jitter} and (not $final_dstsrc_stats{jitter} or $final_dstsrc_stats{jitter} < $srcdst_stats->{jitter}));
            $final_dstsrc_stats{min_delay} = $srcdst_stats->{min_delay} if (defined $srcdst_stats->{min_delay} and (not $final_dstsrc_stats{min_delay} or $final_dstsrc_stats{min_delay} > $srcdst_stats->{min_delay}));
            $final_dstsrc_stats{max_delay} = $srcdst_stats->{max_delay} if (defined $srcdst_stats->{max_delay} and (not $final_dstsrc_stats{max_delay} or $final_dstsrc_stats{max_delay} < $srcdst_stats->{max_delay}));
            $final_dstsrc_stats{loss} = $srcdst_stats->{loss} if (defined $srcdst_stats->{loss} and (not $final_dstsrc_stats{loss} or $final_dstsrc_stats{loss} < $srcdst_stats->{loss}));
        }
