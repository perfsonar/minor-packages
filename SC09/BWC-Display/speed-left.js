...
function Speed(options){
    ...
    this.options = {
        ...
        "dataPeriod":       10,      // seconds - how often to poll pS

        "refreshPeriod":    0.500,  // seconds - how often to update screen
        "minDataPeriod":    5,      // seconds - min duration to show one value
        "dataStalePeriod":  60,     // seconds - when to show 'inactive'
    ...
    var estyle = this.ctx.createLinearGradient(0,0,this.wc,0);
    ...
    var fstyle = this.ctx.createLinearGradient(0,0,this.wc,0);
...
Speed.prototype.refresh = function(){
    ...
    var nBars = this.options.numBars - (this.options.numBars * randValue/this.options.maxValue);

    var w = this.wc;
    var h = this.hc;

    var hb = h/this.options.numBars;
    var hl = hb * this.options.percentBar;

    var wb = w/this.options.numBars;
    var wl = wb * this.options.percentBar;
    ...
    if(this.stale){
        this.ctx.translate(this.hc*(1-this.options.staleWidth)*.5,0);
        h *= this.options.staleWidth;
    }
    ...
    for(var i=0; i < nBars; i++){
      this.ctx.rect((wb*i)+(.5*wl),0,wl,h);
    }
