...
sub owdb_prep{
	...
	my @alphas = ();
	if (defined $args{'ALPHAS'}) {
		if (ref($args{'ALPHAS'}) eq "ARRAY") {
			@alphas = @{$args{'ALPHAS'}};
		} else {
			@alphas = ( $args{'ALPHAS'} );
		}
	...
	$owdbh{'RECV_ADDR'} = $args{'RECV_ADDR'};
	$owdbh{'SEND_ADDR'} = $args{'SEND_ADDR'};
...
#	print "Node: $node_name<br>\n";
#	print "Address: $node_address<br>\n";
#	print "SQL: $sql<br>\n";
...
#	print "Returning: ".join(',', @node_ids)."<br>";
...
sub owdb_fetch{
	...
	my $recv_addr = $owdbh->{'RECV_ADDR'};
	my $send_name = $owdbh->{'SEND_NAME'};
	my $send_addr = $owdbh->{'SEND_ADDR'};
...
RETRY:
	if ($owdbh->{'TABLE_DONE'}) {
		...
		($status, $res) = owdb_lookup_node({ dbh => $owdbh->{'DBH'}, node_name => $recv_name, node_address => $recv_addr, table_prefix => $next_prefix });
		...
		($status, $res) = owdb_lookup_node({ dbh => $owdbh->{'DBH'}, node_name => $send_name, node_address => $send_addr, table_prefix => $next_prefix });
		if ($status != 0 or scalar(@$res) == 0) {
			goto RETRY;
		}
		...
		my $where_clause = 'WHERE a.ei>? AND a.si<?';
		if ($recv_ids) {
			$where_clause .= " AND (";
			my $connector = "";
			foreach my $recv_id (@$recv_ids) {
				$where_clause .= $connector."a.recv_id=? AND b.recv_id=?";
				$connector = " OR ";
			}
			$where_clause .= ")";
		}
		...
		if ($send_ids) {
			...
			foreach my $send_id (@$send_ids) {
				$where_clause .= $connector." a.send_id=? AND b.send_id=?";
				$connector = " OR ";
			}
			$where_clause .= ")";
		}
		$where_clause .= " AND a.valid!=0" if ($owdbh->{'ONLY_VALIDATED'});
		$where_clause .= " AND a.tspec_id=? AND b.tspec_id=?" if ($test_id);
...
#		open(OUT2, ">>", "/tmp/output2");
#		print OUT2 $sql;
#		print OUT2 join(',', @args);
#		close(OUT2);

		$owdbh->{'STH'}->execute(@args) || die "Select owdb data $DBI::errstr";
	...
	while(not $session_done){
		unless ($owdbh->{'STH'}->fetch){
			#print "In owdb_fetch($recv_name, $send_name, $first_time, $last_time): fetch empty<br>";
			$owdbh->{'START'} = 0;
			$owdbh->{'TABLE_DONE'} = 1;
			$session_done = 1;

#			open(OUT2, ">>", "/tmp/output2");
#			print OUT2 "Fetch failed\n";
#			close(OUT2);
...
#		open(OUT2, ">>", "/tmp/output2");
#		print OUT2 "Fetch succeeded: ".Dumper($owdbh)."\n";
#		close(OUT2);

		#print "In owdb_fetch($recv_name, $send_name, $first_time, $last_time): fetch not empty<br>";

		if($owdbh->{'START'} != $owdbh->{'OSTART'}){
#			open(OUT2, ">>", "/tmp/output2");
#			print OUT2 "Resetting session\n";
#			close(OUT2);
...
#				open(OUT2, ">>", "/tmp/output2");
#				print OUT2 "Result: ".Dumper($owhash)."\n";
#				close(OUT2);
...
#		$owdbh->{'OEND'} = new Math::BigInt($owdbh->{'END'});
		$owdbh->{'OSTART'} = $owdbh->{'START'};
		$owdbh->{'OEND'} = $owdbh->{'END'};
		...
		$owdbh->{'ALPHASUM'} += $owdbh->{'Bn'};

		my $sum = $owdbh->{'ALPHASUM'};
		...
		if ($sum > $sent) {
#			open(OUT2, ">>", "/tmp/output2");
#			print OUT2 "START: ".$owdbh->{'START'}."\n";
#			print OUT2 "TEST_ID: ".$owdbh->{'TEST_ID'}."\n";
#			print OUT2 "RECV_ID: ".$owdbh->{'RECV_ID'}."\n";
#			print OUT2 "SEND_ID: ".$owdbh->{'SEND_ID'}."\n";
#			print OUT2 "INDEX: $index\n";
#			print OUT2 "NUM_ALPHAS: $num_alphas\n";
#			print OUT2 "SUM: $sum\n";
#			print OUT2 "Bn: ".$owdbh->{'Bn'}."\n";
#			print OUT2 "SENT: $sent\n";
#			print OUT2 "ALPHAS: ".Dumper($avref)."\n";
#			close(OUT2);
		}
...
#		print OUT2 "ALPHADELAYS: ".Dumper($adref)."\n";
#		close(OUT2);
