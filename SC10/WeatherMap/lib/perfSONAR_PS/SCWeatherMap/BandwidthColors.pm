package perfSONAR_PS::SCWeatherMap::BandwidthColors;
...
sub run {
	...
	foreach my $measurement_result (@{ $link->{measurement_results} }) {
		next unless ($measurement_result->{type} eq "last_throughput");
...
sub parse_colors {
    ...
    foreach my $color_type (@$colors) {
        next unless ($color_type->{type} eq "bandwidth");

	unless ($color_type->{value}) {
		return (-1, "No color values for bandwidth colors");
	}
	...
	if (ref($color_type->{value}) ne "ARRAY") {
        ...
        foreach my $value (@{ $color_type->{value} }) {
                ...
                unless ($value->{color}) {
                        return(-1, "No color specified in bandwidth colors");
                }
