...
sub run {
	...
	# John's code returns in bits per second
	$sourcedest_bps /= 8 if ($sourcedest_bps);
	$destsource_bps /= 8 if ($destsource_bps);
...
sub callSNMP_MA {
    ...
    foreach my $d ( @{ $ma_result->{"data"} } ) {
		...
		$self->{LOGGER}->debug("Got valid time: ".$dt->getAttribute("timeValue"));
		    ...
		    $datum{ $dt->getAttribute("timeValue") } = $data_value;
    ...
    my $maxValue;
    foreach my $ts ( sort keys %datum ) {
        next if lc( $datum{$ts} ) eq "nan";
	...
	next if (scalar(@num_stack) < $pop_point);

	my $avg_value = $current_sum/scalar(@num_stack);

	$maxValue = $avg_value if (not $maxValue or $avg_value > $maxValue);

	# remove the tail
	my $old_value = shift(@num_stack);
	$current_sum -= $old_value;
    }

    $self->{LOGGER}->debug("Max Value: ".$maxValue);
