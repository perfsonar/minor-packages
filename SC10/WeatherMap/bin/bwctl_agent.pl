...
use perfSONAR_PS::SCWeatherMap::perfSONARBUOYBWCTL;
use perfSONAR_PS::SCWeatherMap::BandwidthColors;
...
$file = $confdir."/bwctl_configuration.conf" unless ($file);
...
my $perfsonarbuoy_filter = perfSONAR_PS::SCWeatherMap::perfSONARBUOYBWCTL->new();
($status, $res) = $perfsonarbuoy_filter->init(\%configuration);
if ($status != 0) {
        die($res);
}

my $color_filter = perfSONAR_PS::SCWeatherMap::BandwidthColors->new();
