...
package perfSONAR_PS::WeatherMap::Utilization::DCN;
...
sub init {
    ...
    unless ($conf->{'dcn'} and $conf->{'dcn'}->{'dcn-link'}) {
        return (0, "");
    }

    my ($status, $res) = parse_dcn_links($conf{'dcn'}->{'dcn-link'});
    if ($status != 0) {
        return ($status, $res);
    }
...
sub run {
    ...
    foreach my $idc (@{ $self->{IDCS} }) {
        my $reservations_client = perfSONAR_PS::Client::OSCARS->new();
        $status = $reservations_client->init({ idc_url => $idc, oscars_client => $self->{OSCARS_CLIENT}, axis2_home => $self->{AXIS2_HOME}, java_directory => $conf{JAVA_DIRECTORY} });
        if ($status != 0) {
            $self->{LOGGER}->error("Couldn't initialize oscars client for $idc");
            next;
        }
        ...
        if ($@) {
            $self->{LOGGER}->error("Problem querying for reservations for $idc: ".$@);
            next;
        }
        unless ($status == 0) {
        ...
        foreach my $resv (@{ $res->resDetails }) {
            ...
            foreach my $hop (@{ $resv->pathInfo->path->hop }) {
                ...
                $self->{LOGGER}->debug("Hop has link");

                my $link_id = $hop->link->_id->__value;

                my $link_info = $self->{LINKS}->{$link_id};

                next unless ($link_info);

                $self->{LOGGER}->debug("Hop has link info");
                ...
                if ($link_info->{measurement_parameters}) {
                    ...
                    $MA        = $link_info->{measurement_parameters}->{MA};
                    $direction = $link_info->{measurement_parameters}->{direction};
                    $hostName  = $link_info->{measurement_parameters}->{snmp}->{hostname};
                    $ifName    = $link_info->{measurement_parameters}->{snmp}->{ifname};
                    $ifIndex   = $link_info->{measurement_parameters}->{snmp}->{ifindex};

                    if ($link_info->{measurement_parameters}->{snmp}->{vlanMapping}) {
                        my $vlanMapping = $link_info->{measurement_parameters}->{snmp}->{vlanMapping};
                ...
                foreach my $action (@{ $link_info->{actions} }) {
                    ...
                    if ($action->{'subject'}->{'link'}) {
                        ...
                        if ($prev_edge_point and $source ne $prev_edge_point->{name}) {
                            my @measurements = ();
                            if ($prev_edge_point->{'hostName'} and $prev_edge_point->{'direction'} and ($prev_edge_point->{'ifName'} or $prev_edge_point->{'ifIndex'})) {
...
sub parse_dcn_link_measurement {
    ...
    $measurement_info{direction} = $measurement->{direction}?$measurement->{direction}:"forward";
    $measurement_info{MA} = $measurement->{MA};
    $measurement_info{snmp} = { hostname => $hostname, ifindex => $ifindex, ifname => $ifname, vlanMapping => $vlanMapping };
